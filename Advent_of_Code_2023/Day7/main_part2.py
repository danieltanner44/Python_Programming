...
def reading_input_data(f, card_values):
    ...
    for each_line in f:
        ...
        print(temp)
        print(temp_hand_max_min, "efe")
        hand_min = np.append(hand_min, min(temp_hand_max_min))
        hand_max = np.append(hand_max, max(temp_hand_max_min))
        hand_min_index = np.append(temp_hand_max_min.rfind(hand_min))
        hand_max_index = np.append(temp_hand_max_min.find(hand_max))
        print(temp_hand_max_min.index(max(temp_hand_max_min)))
        print(hand_max, hand_min)
        if only_once == False:
            data = temp
            only_once = True
        else:
    ...
    return data, counter, hand_min, hand_max
...
def main():
    ...
    print(" ")
    card_values = {"A": 13, "K": 12, "Q": 11, "J": 10 * (1 -  joker), "T": 9, "9": 8, "8": 7, "7": 6, "6": 5, "5": 4, "4": 3, "3": 2, "2": 1}
    f = open(r'D:\Advent_of_Code\Advent_of_Code_2023\Day7\Puzzle_Input_d.txt', 'r')
    data, number_of_hands, hand_min, hand_max = reading_input_data(f, card_values)
    ...
    for hands in range(number_of_hands):
        ...
        if hand_type[hands] == 0:
            # Just highest card!
            data[] = data[] # Make lowest card == highest card
            hand_type[hands] = 1
