...
def build_travel_map(initial_data):
    ...
    current_locations = [(travel_map.shape[0] // 2, travel_map.shape[1] // 2),
                         (travel_map.shape[0] // 2, travel_map.shape[1] // 2)] # [santa, robosanta]
    travel_map[current_locations[0]] = 2 # They both start at same location
    for index, direction in enumerate(initial_data):
        if index%2 == 0: # Even is santa
            current_location = current_locations[0]
        else: # Else it is robosanta
            current_location = current_locations[1]
        if direction == "^":
        ...
        if 0 <= new_location[0] < travel_map.shape[0] and 0 <= new_location[1] < travel_map.shape[1]:
            ...
            if index % 2 == 0:  # Even is santa
                current_locations[0] = new_location
            else:  # Else it is robosanta
                current_locations[1] = new_location
        else:
...
def main():
    ...
    print(" ")
    print("==============================================================")
    print("Santa and his robot helper visit "+str(len(np.where(travel_map >= 1)[0]))+" houses at least once!")
    print("==============================================================")
    ts1 = time.time()
    print(" ")
