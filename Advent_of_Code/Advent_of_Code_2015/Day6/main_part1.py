...
def process_lights(instruction, lights_affected):
    ...
    for index, action in enumerate(instruction):
        if action == "on":
            ...
            light_map[lights_affected[index][0][1]:lights_affected[index][1][1]+1,lights_affected[index][0][0]:lights_affected[index][1][0]+1] = 0
        elif action == "toggle":
            light_map[lights_affected[index][0][1]:lights_affected[index][1][1]+1,lights_affected[index][0][0]:lights_affected[index][1][0]+1] = (toggle(light_map[lights_affected[index][0][1]:lights_affected[index][1][1]+1,lights_affected[index][0][0]:lights_affected[index][1][0]+1]))
        else:
            print("Data input error!!!")
    plot_array(light_map)
    number_of_lights_lit = np.sum(light_map)
...
def main():
    ...
    number_of_lights_lit = process_lights(instruction, lights_affected)
    print(" ")
    print("==============================================================")
    print("The number of lights lit is:",number_of_lights_lit)
    print("==============================================================")
    ts1 = time.time()
    print(" ")
