...
def detect_reflections(data_blocks):
    ...
    for i in range(1,np.shape(data_blocks)[1]): # Columns
        ...
        if np.array_equal(data_blocks[:,i-dist:i], np.fliplr(data_blocks[:,i:i+dist])): # captures all the reflections with an axis between columns
    ...
    while finished_reading == False:
        ...
        for i in [0,1]:
            if i == 0: # detect column mirrors
                reflection_index = detect_reflections(data_block)
                ...
                reflection_index = detect_reflections(data_block_t)
