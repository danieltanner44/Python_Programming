...
def read_input_data(filename):
    with open(filename, 'r') as f:
        ...
        initial_states = np.array(initial_states, dtype=np.int8)
    return initial_states

def simulate_lanternfish_population(initial_states, days_to_model):
    population_model = np.array([np.int16(each) for each in initial_states], dtype=np.int8)
    for days in range(1, days_to_model + 1):
        population_model -= 1
        pop_check_indices = np.where(population_model == -1)[0]
        progress_bar(days, days_to_model)
        population_model = np.concatenate((population_model, np.array([8]*len(pop_check_indices),np.int8)))
