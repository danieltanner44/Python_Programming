...
def intercode_computer(instructions, computer_input):
    ...
    while True:
        ...
        if opcode in [1,2]:     # These opcodes have 3 parameters, e.g., [1,10,20,30]
            num_parameters = 3
        ...
        if opcode == 1:     # If opcode is 1 then program uses addition logic
            ...
            diagnostic_output.append(update_sequence[current_instruction[1]])
            index += 2      # After each iteration move forward 4 indexes to the next opcode

        print(f"Iteration {index}: opcode = {opcode}, instruction = {current_instruction}, parameter modes are = {parameter_modes}")
