...
def process_combinations(input_range):
    ...
    for pass_candidate in range(int(input_range[0]), int(input_range[1]) + 1):
        ...
        duplicate_checker = np.zeros(shape=[10], dtype=np.int8)
        for j, digit in enumerate(pass_candidate[:-1]):
            if digit == pass_candidate[j + 1]:
                # Duplicate adjacent digit found
                duplicate_checker[digit] += 1
                last_duplicate_digit = digit
            if digit <= pass_candidate[j + 1]:
        ...
        if len(np.where(duplicate_checker == 1)[0]) > 0:
            # There is an adjacent duplicate pair in the list
...
def main():
    ...
    print(" ")
    print("==============================================================")
    print("PART 1: The number of valid passcodes is:", number_of_valid_passcodes) # 944 too low
    print("==============================================================")
    print(" ")
